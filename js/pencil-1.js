// js/women-pencilskirts.js
// Interactions for Pencil Skirts page:
// - Lazy image loading
// - Category filter and sort
// - Size validation and order via WhatsApp
// - Preview modal (created on demand)
// - Safe "load more" that preserves grid column structure
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#pencilskirts') || document;
  var ROW = qs('#pencilskirts .row', ROOT) || qs('.row', ROOT);
  ...
  /* Filtering and sorting (non-destructive) */
  function applyFilters() {
    ...
    // Sort visible columns safely by re-appending columns to the row
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="pencil-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="pencil-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="pencil-preview-title"></h3>'
      + '    <p id="pencil-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="pencil-preview-order" class="btn btn-brown">Order this skirt</button>'
      + '      <button id="pencil-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#pencil-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#pencil-preview-img', overlay);
    var previewTitle = qs('#pencil-preview-title', overlay);
    var previewText = qs('#pencil-preview-text', overlay);
    var previewOrder = qs('#pencil-preview-order', overlay);
    var previewContact = qs('#pencil-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Pencil skirt preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'pencil skirt') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Pencil skirt';
