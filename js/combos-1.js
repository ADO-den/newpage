// js/men-combos-1.js
// Interactions for Men Combos page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#combos-grid') || document;
  var ROW = qs('#combos-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="combo-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="combo-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="combo-preview-title"></h3>'
      + '    <p id="combo-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="combo-preview-order" class="btn btn-brown">Order this combo</button>'
      + '      <button id="combo-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#combo-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#combo-preview-img', overlay);
    var previewTitle = qs('#combo-preview-title', overlay);
    var previewText = qs('#combo-preview-text', overlay);
    var previewOrder = qs('#combo-preview-order', overlay);
    var previewContact = qs('#combo-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Combo preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello, I would like to ask about the "' + (title || 'combo') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Combo';
