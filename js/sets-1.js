// js/men-sets-1.js
// Interactions for Men Sets page:
// - Lazy image loading
...
(function () {
  ...
  /* Elements */
  var ROOT = qs('#sets-grid') || document;
  var ROW = qs('#sets-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="sets-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="sets-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="sets-preview-title"></h3>'
      + '    <p id="sets-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="sets-preview-order" class="btn btn-brown">Order this set</button>'
      + '      <button id="sets-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#sets-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#sets-preview-img', overlay);
    var previewTitle = qs('#sets-preview-title', overlay);
    var previewText = qs('#sets-preview-text', overlay);
    var previewOrder = qs('#sets-preview-order', overlay);
    var previewContact = qs('#sets-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Set preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello, I would like to ask about the "' + (title || 'set') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Set';
