// js/women-tiered-1.js
// Interactions for Tiered Tops page:
// - Lazy image loading
...
(function () {
  ...
  var ROOT = qs('#tiered') || document;
  var ROW = qs('#tiered .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="tier-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="tier-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="tier-preview-title"></h3>'
      + '    <p id="tier-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="tier-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="tier-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#tier-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#tier-preview-img', overlay);
    var previewTitle = qs('#tier-preview-title', overlay);
    var previewText = qs('#tier-preview-text', overlay);
    var previewOrder = qs('#tier-preview-order', overlay);
    var previewContact = qs('#tier-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'tiered top') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Tiered top';
