// js/men-shirts-1.js
// Interactions for Men Shirts page:
// - Lazy image loading
...
(function () {
  ...
  var ROOT = qs('#shirts-grid') || document;
  var ROW = qs('#shirts-grid .row', ROOT) || qs('.row', ROOT);
  var FILTER_BTNS = qsa('[data-filter]', ROOT);
  var SORT_SELECT = qs('#sort-select', ROOT);
  var LOAD_MORE = qs('#load-more', ROOT);
  var DEFAULT_PHONE = '23436754060';
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="shirts-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="shirts-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="shirts-preview-title"></h3>'
      + '    <p id="shirts-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="shirts-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="shirts-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#shirts-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#shirts-preview-img', overlay);
    var previewTitle = qs('#shirts-preview-title', overlay);
    var previewText = qs('#shirts-preview-text', overlay);
    var previewOrder = qs('#shirts-preview-order', overlay);
    var previewContact = qs('#shirts-preview-contact', overlay);
