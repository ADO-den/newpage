// js/women-wrap-1.js
// Interactions for Wrap Tops page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#wrap') || document;
  var ROW = qs('#wrap .row', ROOT) || qs('.row', ROOT);
  ...
  function applyFilters() {
    ...
    // Sort: re-append columns in chosen order
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '      <button id="wrap-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="wrap-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
  ...
  function openPreview(card) {
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'wrap top') + '".');
    };
