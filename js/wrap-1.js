// js/women-wrap-1.js
// Interactions for Wrap Dresses page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#wrap-grid') || document;
  var ROW = qs('#wrap-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="wrap-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="wrap-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="wrap-preview-title"></h3>'
      + '    <p id="wrap-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="wrap-preview-order" class="btn btn-brown">Order this dress</button>'
      + '      <button id="wrap-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#wrap-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#wrap-preview-img', overlay);
    var previewTitle = qs('#wrap-preview-title', overlay);
    var previewText = qs('#wrap-preview-text', overlay);
    var previewOrder = qs('#wrap-preview-order', overlay);
    var previewContact = qs('#wrap-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello, I would like to ask about the "' + (title || 'wrap dress') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Wrap Dress';
