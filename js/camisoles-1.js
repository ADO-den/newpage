// js/women-camisoles-1.js
// Interactions for Camisoles page:
// - Lazy image loading
...
(function () {
  ...
  var ROOT = qs('#camisoles') || document;
  var ROW = qs('#camisoles .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="cam-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="cam-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="cam-preview-title"></h3>'
      + '    <p id="cam-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="cam-preview-order" class="btn btn-brown">Order this camisole</button>'
      + '      <button id="cam-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#cam-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#cam-preview-img', overlay);
    var previewTitle = qs('#cam-preview-title', overlay);
    var previewText = qs('#cam-preview-text', overlay);
    var previewOrder = qs('#cam-preview-order', overlay);
    var previewContact = qs('#cam-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Camisole preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'camisole') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Camisole';
