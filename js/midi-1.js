// js/women-midi-1.js
// Interactions for Midi Dresses page:
// - Lazy image loading
...
(function () {
  ...
  /* Elements */
  var ROOT = qs('#midi-grid') || document;
  var ROW = qs('#midi-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="midi-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="midi-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="midi-preview-title"></h3>'
      + '    <p id="midi-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="midi-preview-order" class="btn btn-brown">Order this dress</button>'
      + '      <button id="midi-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#midi-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#midi-preview-img', overlay);
    var previewTitle = qs('#midi-preview-title', overlay);
    var previewText = qs('#midi-preview-text', overlay);
    var previewOrder = qs('#midi-preview-order', overlay);
    var previewContact = qs('#midi-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Dress preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello, I would like to ask about the "' + (title || 'midi dress') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Midi Dress';
  ...
  window.__womenMidi = { applyFilters: applyFilters, openPreview: openPreview, closePreview: closePreview };
