...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#boots') || document;
  var ROW = qs('#boots .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="boot-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="boot-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="boot-preview-title"></h3>'
      + '    <p id="boot-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="boot-preview-order" class="btn btn-brown">Order this pair</button>'
      + '      <button id="boot-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#boot-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#boot-preview-img', overlay);
    var previewTitle = qs('#boot-preview-title', overlay);
    var previewText = qs('#boot-preview-text', overlay);
    var previewOrder = qs('#boot-preview-order', overlay);
    var previewContact = qs('#boot-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Boot preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'boot') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Boots';
  ...
  window.__womenBoots = { applyFilters: applyFilters, openPreview: openPreview, closePreview: closePreview };
