// js/women-maxi-1.js
// Interactions for Maxi Dresses page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#maxi-grid') || document;
  var ROW = qs('#maxi-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="maxi-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="maxi-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="maxi-preview-title"></h3>'
      + '    <p id="maxi-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="maxi-preview-order" class="btn btn-brown">Order this dress</button>'
      + '      <button id="maxi-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#maxi-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#maxi-preview-img', overlay);
    var previewTitle = qs('#maxi-preview-title', overlay);
    var previewText = qs('#maxi-preview-text', overlay);
    var previewOrder = qs('#maxi-preview-order', overlay);
    var previewContact = qs('#maxi-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello, I would like to ask about the "' + (title || 'maxi dress') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Maxi Dress';
