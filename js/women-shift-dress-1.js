// js/women-shift-dress-1.js
// Interactions for Women Shift Dress page:
// - Lazy image loading
// - Category filter and sort
// - Size validation and WhatsApp order
// - Preview modal created on demand
// - Safe "load more" cloning
...
(function () {
  ...
  // Normalize phone and open WhatsApp
  function normalizePhone(raw) {
    if (!raw) return null;
    var digits = String(raw).replace(/[^\d]/g, '');
    if (!digits) return null;
    if (digits.length >= 10 && digits[0] !== '0') return digits;
    if (digits.length >= 10 && digits[0] === '0') return '234' + digits.replace(/^0+/, '');
    return null;
  }
  function openWhatsApp(phone, message) {
    var normalized = normalizePhone(phone) || '2348136754060';
    var url = 'https://wa.me/' + normalized + (message ? ('?text=' + encodeURIComponent(message)) : '');
    try { window.open(url, '_blank'); } catch (err) { window.location.href = url; }
  }

  var ROOT = qs('#shift-grid') || document;
  var ROW = qs('#shift-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function applyFilters() {
    ...
    var cols = Array.prototype.slice.call(ROW.querySelectorAll('[class*="col-"]')).filter(function (col) {
    ...
    if (sortVal && visibleCols.length && ROW) {
      var mapped = visibleCols.map(function (col) {
        ...
        var isNew = card.getAttribute('data-newest') === 'true' || card.getAttribute('data-new') === 'true';
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="shift-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="shift-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="shift-preview-title"></h3>'
      + '    <p id="shift-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="shift-preview-order" class="btn btn-primary">Order this item</button>'
      + '      <button id="shift-preview-contact" class="btn btn-outline">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#shift-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#shift-preview-img', overlay);
    var previewTitle = qs('#shift-preview-title', overlay);
    var previewText = qs('#shift-preview-text', overlay);
    var previewOrder = qs('#shift-preview-order', overlay);
    var previewContact = qs('#shift-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp('2348136754060', 'Hello, I would like to ask about the "' + (title || 'item') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var messageParts = [
        ...
        'Buyer: [Your name] | Address: [Delivery address]'
      ].filter(Boolean);
