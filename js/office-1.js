// js/men-office-1.js
// Interactions for Men Office Wear page:
// - Lazy image loading
...
(function () {
  ...
  /* Page elements */
  var ROOT = qs('#office-grid') || document;
  var ROW = qs('#office-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="office-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="office-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="office-preview-title"></h3>'
      + '    <p id="office-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="office-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="office-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#office-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#office-preview-img', overlay);
    var previewTitle = qs('#office-preview-title', overlay);
    var previewText = qs('#office-preview-text', overlay);
    var previewOrder = qs('#office-preview-order', overlay);
    var previewContact = qs('#office-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello, I would like to ask about the "' + (title || 'office item') + '".');
    };
