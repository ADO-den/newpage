// js/women-miniskirts.js
// Interactions for Mini Skirts page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#miniskirts') || document;
  var ROW = qs('#miniskirts .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="mini-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="mini-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="mini-preview-title"></h3>'
      + '    <p id="mini-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="mini-preview-order" class="btn btn-brown">Order this skirt</button>'
      + '      <button id="mini-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#mini-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#mini-preview-img', overlay);
    var previewTitle = qs('#mini-preview-title', overlay);
    var previewText = qs('#mini-preview-text', overlay);
    var previewOrder = qs('#mini-preview-order', overlay);
    var previewContact = qs('#mini-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Mini skirt preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'mini skirt') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Mini skirt';
