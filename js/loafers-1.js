// js/men-loafers-1.js
// Interactions for Men Loafers page:
// - Lazy image loading
...
(function () {
  ...
  var ROOT = qs('#loafers-grid') || document;
  var ROW = qs('#loafers-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="loafers-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="loafers-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="loafers-preview-title"></h3>'
      + '    <p id="loafers-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="loafers-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="loafers-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#loafers-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#loafers-preview-img', overlay);
    var previewTitle = qs('#loafers-preview-title', overlay);
    var previewText = qs('#loafers-preview-text', overlay);
    var previewOrder = qs('#loafers-preview-order', overlay);
    var previewContact = qs('#loafers-preview-contact', overlay);
