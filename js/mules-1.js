...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#mules') || document;
  var ROW = qs('#mules .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="mule-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="mule-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="mule-preview-title"></h3>'
      + '    <p id="mule-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="mule-preview-order" class="btn btn-brown">Order this pair</button>'
      + '      <button id="mule-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#mule-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#mule-preview-img', overlay);
    var previewTitle = qs('#mule-preview-title', overlay);
    var previewText = qs('#mule-preview-text', overlay);
    var previewOrder = qs('#mule-preview-order', overlay);
    var previewContact = qs('#mule-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Mule preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'mule') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Mules';
