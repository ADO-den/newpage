// js/women-buttonup.js
// Interactions for Button-Up Shirts page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#buttonup') || document;
  var ROW = qs('#buttonup .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="btnup-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="btnup-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="btnup-preview-title"></h3>'
      + '    <p id="btnup-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="btnup-preview-order" class="btn btn-brown">Order this shirt</button>'
      + '      <button id="btnup-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#btnup-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#btnup-preview-img', overlay);
    var previewTitle = qs('#btnup-preview-title', overlay);
    var previewText = qs('#btnup-preview-text', overlay);
    var previewOrder = qs('#btnup-preview-order', overlay);
    var previewContact = qs('#btnup-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Shirt preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'shirt') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Button-up shirt';
