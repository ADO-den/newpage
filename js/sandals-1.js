// js/women-sandals-1.js
// Interactions for Sandals page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#sandals') || document;
  var ROW = qs('#sandals .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="sand-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="sand-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="sand-preview-title"></h3>'
      + '    <p id="sand-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="sand-preview-order" class="btn btn-brown">Order this pair</button>'
      + '      <button id="sand-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#sand-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#sand-preview-img', overlay);
    var previewTitle = qs('#sand-preview-title', overlay);
    var previewText = qs('#sand-preview-text', overlay);
    var previewOrder = qs('#sand-preview-order', overlay);
    var previewContact = qs('#sand-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Sandal preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'sandal') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Sandals';
