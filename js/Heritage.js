// js/women-Heritage.js
// Interactions for Heritage Collection page:
// - Lazy image loading
...
(function () {
  ...
  /* DOM refs */
  var grid = qs('#heritage .row') || qs('#heritage');
  ...
  function getProductsColumns() {
    // Return column wrappers that contain product cards to preserve grid structure
    var cols = qsa('#heritage .row > [class*="col-"]');
    return cols.filter(function (col) { return !!col.querySelector('.product-card'); });
  }

  function getProducts() {
    var cols = getProductsColumns();
    return cols.map(function (col) { return col.querySelector('.product-card'); }).filter(Boolean);
  }

  /* Lazy load images (IntersectionObserver) */
  function initLazy() {
    var imgs = qsa('#heritage img.lazy');
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="heritage-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="heritage-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="heritage-preview-title"></h3>'
      + '    <p id="heritage-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="heritage-preview-order" class="btn btn-brown">Order this piece</button>'
      + '      <button id="heritage-preview-contact" class="btn btn-outline-brown">Contact Heritage Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#heritage-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#heritage-preview-img', previewOverlay);
    var previewTitle = qs('#heritage-preview-title', previewOverlay);
    var previewText = qs('#heritage-preview-text', previewOverlay);
    var previewOrder = qs('#heritage-preview-order', previewOverlay);
    var previewContact = qs('#heritage-preview-contact', previewOverlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp('2348136754060', 'Hello Heritage Team, I would like to ask about the "' + (title || 'piece') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Heritage piece';
