// js/women-blouses-1.js
// Interactions for the Blouses (Top Section) page:
// - Lazy image loading
...
(function () {
  ...
  var ROOT = qs('#blouses') || document;
  var ROW = qs('#blouses .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="blouse-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="blouse-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="blouse-preview-title"></h3>'
      + '    <p id="blouse-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="blouse-preview-order" class="btn btn-brown">Order this blouse</button>'
      + '      <button id="blouse-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#blouse-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#blouse-preview-img', overlay);
    var previewTitle = qs('#blouse-preview-title', overlay);
    var previewText = qs('#blouse-preview-text', overlay);
    var previewOrder = qs('#blouse-preview-order', overlay);
    var previewContact = qs('#blouse-preview-contact', overlay);

    if (imgEl) {
      var src = imgEl.getAttribute('data-src') || imgEl.src || '';
      previewImg.src = src;
      previewImg.alt = title || 'Blouse preview';
    }
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'blouse') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Blouse';
