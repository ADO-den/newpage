// js/women-ruffled-1.js
// Interactions for Ruffled Tops page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#ruffled') || document;
  var ROW = qs('#ruffled .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="ruff-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="ruff-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="ruff-preview-title"></h3>'
      + '    <p id="ruff-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="ruff-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="ruff-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#ruff-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#ruff-preview-img', overlay);
    var previewTitle = qs('#ruff-preview-title', overlay);
    var previewText = qs('#ruff-preview-text', overlay);
    var previewOrder = qs('#ruff-preview-order', overlay);
    var previewContact = qs('#ruff-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'item') + '".');
    };
