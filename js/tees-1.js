// js/men-tees-vneck.js
// Interactions for Men Tees V Neck page:
// - Lazy image loading
...
(function () {
  ...
  var ROOT = qs('#vneck-grid') || document;
  var ROW = qs('#vneck-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="vneck-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="vneck-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="vneck-preview-title"></h3>'
      + '    <p id="vneck-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="vneck-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="vneck-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#vneck-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#vneck-preview-img', overlay);
    var previewTitle = qs('#vneck-preview-title', overlay);
    var previewText = qs('#vneck-preview-text', overlay);
    var previewOrder = qs('#vneck-preview-order', overlay);
    var previewContact = qs('#vneck-preview-contact', overlay);
