// js/women-offshoulders.js
// Interactions for Off-Shoulders page:
// - Lazy image loading
// - Category filter and sort
// - Size validation and order via WhatsApp
// - Preview modal (created on demand)
// - Safe "load more" preserving grid columns
...
(function () {
  ...
  var ROOT = qs('#offshoulders') || document;
  var ROW = qs('#offshoulders .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="off-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="off-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="off-preview-title"></h3>'
      + '    <p id="off-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="off-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="off-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#off-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#off-preview-img', overlay);
    var previewTitle = qs('#off-preview-title', overlay);
    var previewText = qs('#off-preview-text', overlay);
    var previewOrder = qs('#off-preview-order', overlay);
    var previewContact = qs('#off-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'off-shoulder top') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Off-shoulder top';
