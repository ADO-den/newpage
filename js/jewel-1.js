...
(function () {
  ...
  var ROOT = qs('#jewelry-grid') || document;
  var ROW = qs('#jewelry-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="jewelry-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="jewelry-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="jewelry-preview-title"></h3>'
      + '    <p id="jewelry-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="jewelry-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="jewelry-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#jewelry-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#jewelry-preview-img', overlay);
    var previewTitle = qs('#jewelry-preview-title', overlay);
    var previewText = qs('#jewelry-preview-text', overlay);
    var previewOrder = qs('#jewelry-preview-order', overlay);
    var previewContact = qs('#jewelry-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello, I would like to ask about the "' + (title || 'item') + '".');
    };
  ...
  if (LOAD_MORE) {
    LOAD_MORE.addEventListener('click', function () {
      ...
      setTimeout(function () {
        ...
        // Clone first 4 items to simulate loading more. Replace with real data when available.
