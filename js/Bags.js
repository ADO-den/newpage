// js/Bags.js
// Lightweight interactions for Bags page:
// - Lazy image loading
...
(function () {
  ...
  // Elements
  var ROOT = qs('#bags-grid') || document;
  var ROW = qs('#bags-grid .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="bags-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="bags-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="bags-preview-title"></h3>'
      + '    <p id="bags-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="bags-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="bags-preview-contact" class="btn btn-outline-brown">Contact Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#bags-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#bags-preview-img', overlay);
    var previewTitle = qs('#bags-preview-title', overlay);
    var previewText = qs('#bags-preview-text', overlay);
    var previewOrder = qs('#bags-preview-order', overlay);
    var previewContact = qs('#bags-preview-contact', overlay);
  ...
  if (LOAD_MORE) {
    LOAD_MORE.addEventListener('click', function () {
      ...
      setTimeout(function () {
        ...
        // Clone first 4 items to simulate loading more. Update or replace with real data as needed.
