...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#flats') || document;
  var ROW = qs('#flats .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="flat-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="flat-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="flat-preview-title"></h3>'
      + '    <p id="flat-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="flat-preview-order" class="btn btn-brown">Order this pair</button>'
      + '      <button id="flat-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#flat-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#flat-preview-img', overlay);
    var previewTitle = qs('#flat-preview-title', overlay);
    var previewText = qs('#flat-preview-text', overlay);
    var previewOrder = qs('#flat-preview-order', overlay);
    var previewContact = qs('#flat-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'flat') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Flats';
