// js/women-CropTops.js
// Interactions for Crop Tops page:
// - Lazy image loading
...
(function () {
  ...
  /* Page selectors */
  var ROOT = qs('#croptops') || document;
  var ROW = qs('#croptops .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="crop-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="crop-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="crop-preview-title"></h3>'
      + '    <p id="crop-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="crop-preview-order" class="btn btn-brown">Order this item</button>'
      + '      <button id="crop-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#crop-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#crop-preview-img', overlay);
    var previewTitle = qs('#crop-preview-title', overlay);
    var previewText = qs('#crop-preview-text', overlay);
    var previewOrder = qs('#crop-preview-order', overlay);
    var previewContact = qs('#crop-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'crop top') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Crop top';
