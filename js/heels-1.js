...
(function () {
  ...
  /* Page elements */
  var ROOT = qs('#heels') || document;
  var ROW = qs('#heels .row', ROOT) || qs('.row', ROOT);
  ...
  function createPreview() {
    ...
    overlay.innerHTML = ''
      ...
      + '  <button id="heel-preview-close" aria-label="Close preview" style="position:absolute;right:12px;top:12px;background:transparent;border:0;font-size:22px;cursor:pointer;">✕</button>'
      + '  <img id="heel-preview-img" src="" alt="" />'
      + '  <div class="preview-body">'
      + '    <h3 id="heel-preview-title"></h3>'
      + '    <p id="heel-preview-text" class="meta"></p>'
      + '    <div style="margin-top:12px;display:flex;gap:8px;">'
      + '      <button id="heel-preview-order" class="btn btn-brown">Order this pair</button>'
      + '      <button id="heel-preview-contact" class="btn btn-outline-brown">Contact Tailoring Team</button>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    ...
    qs('#heel-preview-close', overlay).addEventListener('click', closePreview);
  ...
  function openPreview(card) {
    ...
    var previewImg = qs('#heel-preview-img', overlay);
    var previewTitle = qs('#heel-preview-title', overlay);
    var previewText = qs('#heel-preview-text', overlay);
    var previewOrder = qs('#heel-preview-order', overlay);
    var previewContact = qs('#heel-preview-contact', overlay);
    ...
    previewContact.onclick = function () {
      openWhatsApp(DEFAULT_PHONE, 'Hello Tailoring Team, I would like to ask about the "' + (title || 'heel') + '".');
    };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (orderBtn) {
      ...
      var title = (card && (card.querySelector('.card-title') || {})).textContent || 'Heels';
